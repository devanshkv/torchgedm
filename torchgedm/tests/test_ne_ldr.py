"""
Tests for Low Density Region in Q1 (LDRQ1) density component
"""
...
class TestNeLDRQ1:
    """Test suite for Low Density Region component"""
    ...
    def test_ldr_center(self, ne2001_data):
        ...
        ne, F, w = neLDRQ1(x, y, z, ne2001_data)

        # Should be inside at center
        assert ne > 0
        assert ne == ne2001_data.neldr
        assert F == ne2001_data.Fldr
        assert w == 1

    def test_far_from_ldr(self, ne2001_data):
        """Test density far from LDR (should be zero)"""
        x = torch.tensor(5.0)
        y = torch.tensor(0.0)
        z = torch.tensor(5.0)
    ...
    def test_batched_computation(self, ne2001_data):
        ...
        # Create grid centered on LDR
        x_center = ne2001_data.xldr
        y_center = ne2001_data.yldr
        z_center = ne2001_data.zldr

        # Use parameters to size grid
        scale = max(ne2001_data.aldr, ne2001_data.bldr, ne2001_data.cldr).item()
        ...
        ne, F, w = neLDRQ1(x_grid, y_grid, z_grid, ne2001_data)
    ...
    def test_ellipsoidal_geometry(self, ne2001_data):
        ...
        ne_center, _, w_center = neLDRQ1(x_center, y_center, z_center, ne2001_data)
        assert ne_center > 0
        assert w_center == 1

    def test_rotation(self, ne2001_data):
        """Test that ellipsoid is rotated in xy-plane"""
        # The rotation angle is thetaldr
        theta = ne2001_data.thetaldr
    ...
    def test_z_symmetry(self, ne2001_data):
        ...
        ne_plus, _, _ = neLDRQ1(x_center, y_center, z_center + dz, ne2001_data)
        ne_minus, _, _ = neLDRQ1(x_center, y_center, z_center - dz, ne2001_data)
    ...
    def test_works_with_requires_grad(self, ne2001_data):
        ...
        x = torch.tensor(ne2001_data.xldr.item(), requires_grad=True)
        y = torch.tensor(ne2001_data.yldr.item(), requires_grad=True)
        z = torch.tensor(ne2001_data.zldr.item(), requires_grad=True)
    ...
    def test_parameter_broadcasting(self, ne2001_data):
        ...
        # 1D input
        x_1d = torch.linspace(0, 2, 50)
        y_1d = torch.full_like(x_1d, 8.0)
        z_1d = torch.zeros_like(x_1d)

        ne_1d, F_1d, w_1d = neLDRQ1(x_1d, y_1d, z_1d, ne2001_data)
        ...
        # 2D input
        x_2d = torch.linspace(0, 2, 10).unsqueeze(1)
        y_2d = torch.linspace(7, 9, 20).unsqueeze(0)
        z_2d = torch.zeros(10, 20)

        ne_2d, F_2d, w_2d = neLDRQ1(x_2d, y_2d, z_2d, ne2001_data)
    ...
    def test_boundary(self, ne2001_data):
        ...
        z_values = torch.linspace(z_center - 2*ne2001_data.cldr,
                                   z_center + 2*ne2001_data.cldr, 50)

        ne_z, _, _ = neLDRQ1(x_center.expand_as(z_values),
    ...
    def test_constant_inside(self, ne2001_data):
        ...
        ne_near, _, w_near = neLDRQ1(x_near, y_near, z_near, ne2001_data)
    ...
    def test_quadrant_location(self, ne2001_data):
        """Test that LDR is in first quadrant (Q1) as name suggests"""
        # The name is "Low Density Region in Q1"
        # Check parameters to see location
        x_center = ne2001_data.xldr
        y_center = ne2001_data.yldr

        # Q1 typically means x > 0, y > 0 in galactic coordinates
        # Just verify we can evaluate the function (location is data-driven)
        ne, F, w = neLDRQ1(x_center, y_center, ne2001_data.zldr, ne2001_data)
        assert ne > 0
        assert w == 1

    def test_low_density(self, ne2001_data):
        """Test that this is indeed a low density region"""
        # LDR should have lower density than typical disk
        # This is encoded in the neldr parameter
        assert ne2001_data.neldr >= 0
        # Just verify it's a reasonable value
        assert ne2001_data.neldr < 1.0  # Low density means < 1 cm^-3
